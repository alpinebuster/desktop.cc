...
#include <coreplugin/homemanager/ieditorfactory_p.h>
...
static const char kMakeWritableWarning[]             = "Core.HomeManager.MakeWritable";

static const char documentStatesKey[]                = "HomeManager/DocumentStates";
static const char reloadBehaviorKey[]                = "HomeManager/ReloadBehavior";
static const char autoSaveEnabledKey[]               = "HomeManager/AutoSaveEnabled";
static const char autoSaveIntervalKey[]              = "HomeManager/AutoSaveInterval";
static const char autoSuspendEnabledKey[]            = "HomeManager/AutoSuspendEnabled";
static const char autoSuspendMinDocumentCountKey[]   = "HomeManager/AutoSuspendMinDocuments";
static const char warnBeforeOpeningBigTextFilesKey[] = "HomeManager/WarnBeforeOpeningBigTextFiles";
static const char bigTextFileSizeLimitKey[]          = "HomeManager/BigTextFileSizeLimitInMB";
static const char maxRecentFilesKey[]                = "HomeManager/MaxRecentFiles";
static const char fileSystemCaseSensitivityKey[]     = "Core/FileSystemCaseSensitivity";
static const char preferredHomeFactoriesKey[]        = "HomeManager/PreferredHomeFactories";
...
*/
HomeManagerPlaceHolder::HomeManagerPlaceHolder(QWidget *parent)
    : QWidget(parent)
{
    setLayout(new QVBoxLayout);
    layout()->setContentsMargins(0, 0, 0, 0);
    setFocusProxy(HomeManagerPrivate::mainEditorArea());
}

/*!
    \internal
*/
HomeManagerPlaceHolder::~HomeManagerPlaceHolder()
{
    // EditorManager will be deleted in ~MainWindow()
    QWidget *em = HomeManagerPrivate::mainEditorArea();
...
/*!
    \internal
*/
void HomeManagerPlaceHolder::showEvent(QShowEvent *)
{
...
static HomeManager *m_instance = nullptr;
static HomeManagerPrivate *d;
...
static void setFocusToEditorViewAndUnmaximizePanes(HomeView *view)
{
...
HomeManagerPrivate::HomeManagerPrivate(QObject *parent) :
    QObject(parent),
    m_revertToSavedAction(new QAction(HomeManager::tr("Revert to Saved"), this)),
    m_saveAction(new QAction(this)),
    m_saveAsAction(new QAction(this)),
    m_closeCurrentEditorAction(new QAction(HomeManager::tr("Close"), this)),
    m_closeAllEditorsAction(new QAction(HomeManager::tr("Close All"), this)),
    m_closeOtherDocumentsAction(new QAction(HomeManager::tr("Close Others"), this)),
    m_closeAllEditorsExceptVisibleAction(new QAction(HomeManager::tr("Close All Except Visible"), this)),
    m_gotoNextDocHistoryAction(new QAction(HomeManager::tr("Next Open Document in History"), this)),
    m_gotoPreviousDocHistoryAction(new QAction(HomeManager::tr("Previous Open Document in History"), this)),
    m_goBackAction(new QAction(Utils::Icons::PREV.icon(), HomeManager::tr("Go Back"), this)),
    m_goForwardAction(new QAction(Utils::Icons::NEXT.icon(), HomeManager::tr("Go Forward"), this)),
    m_gotoLastEditAction(new QAction(HomeManager::tr("Go to Last Edit"), this)),
    m_copyFilePathContextAction(new QAction(HomeManager::tr("Copy Full Path"), this)),
    m_copyLocationContextAction(new QAction(HomeManager::tr("Copy Path and Line Number"), this)),
    m_copyFileNameContextAction(new QAction(HomeManager::tr("Copy File Name"), this)),
    m_saveCurrentEditorContextAction(new QAction(HomeManager::tr("&Save"), this)),
    m_saveAsCurrentEditorContextAction(new QAction(HomeManager::tr("Save &As..."), this)),
    m_revertToSavedCurrentEditorContextAction(new QAction(HomeManager::tr("Revert to Saved"), this)),
    m_closeCurrentEditorContextAction(new QAction(HomeManager::tr("Close"), this)),
    m_closeAllEditorsContextAction(new QAction(HomeManager::tr("Close All"), this)),
    m_closeOtherDocumentsContextAction(new QAction(HomeManager::tr("Close Others"), this)),
    m_closeAllEditorsExceptVisibleContextAction(new QAction(HomeManager::tr("Close All Except Visible"), this)),
...
HomeManagerPrivate::~HomeManagerPrivate()
{
    if (ICore::instance())
    ...
    for (int i = 0; i < m_editorAreas.size(); ++i) {
        HomeArea *area = m_editorAreas.at(i);
        disconnect(area, &QObject::destroyed, this, &HomeManagerPrivate::editorAreaDestroyed);
        delete area;
    }
    m_editorAreas.clear();
...
void HomeManagerPrivate::init()
{
    DocumentModel::init();
    connect(ICore::instance(), &ICore::contextAboutToChange,
            this, &HomeManagerPrivate::handleContextChange);
    connect(qApp, &QApplication::applicationStateChanged,
            this, [](Qt::ApplicationState state) {
                if (state == Qt::ApplicationActive)
                    HomeManager::updateWindowTitles();
            });

    const Context editManagerContext(Constants::C_HOME_MANAGER);
    // combined context for edit & design modes
    const Context editDesignContext(Constants::C_HOME_MANAGER, Constants::C_DESIGN_MODE);
    ...
    connect(m_revertToSavedAction, &QAction::triggered, m_instance, &HomeManager::revertToSaved);

    // Save Action
    ActionManager::registerAction(m_saveAction, Constants::SAVE, editManagerContext);
    connect(m_saveAction, &QAction::triggered, m_instance, []() { HomeManager::saveDocument(); });

    // Save As Action
    ActionManager::registerAction(m_saveAsAction, Constants::SAVEAS, editManagerContext);
    connect(m_saveAsAction, &QAction::triggered, m_instance, &HomeManager::saveDocumentAs);
    ...
    connect(m_closeCurrentEditorAction, &QAction::triggered,
            m_instance, &HomeManager::slotCloseCurrentEditorOrDocument);
    ...
    if (HostOsInfo::isWindowsHost()) {
        ...
        cmd->setDescription(HomeManager::tr("Close"));
    ...
    mfile->addAction(cmd, Constants::G_FILE_CLOSE);
    connect(m_closeAllEditorsAction, &QAction::triggered, m_instance, &HomeManager::closeAllDocuments);
    ...
    connect(m_closeOtherDocumentsAction, &QAction::triggered,
            m_instance, []() { HomeManager::closeOtherDocuments(); });
    ...
    connect(m_closeAllEditorsExceptVisibleAction,
            &QAction::triggered,
            this,
            &HomeManagerPrivate::closeAllEditorsExceptVisible);
    ...
    connect(m_openGraphicalShellAction, &QAction::triggered, this, [] {
        if (!HomeManager::currentDocument())
            return;
        const FilePath fp = HomeManager::currentDocument()->filePath();
        if (!fp.isEmpty())
    ...
    connect(m_openTerminalAction, &QAction::triggered, this, &HomeManagerPrivate::openTerminal);
    connect(m_findInDirectoryAction, &QAction::triggered,
            this, &HomeManagerPrivate::findInDirectory);
    ...
    connect(m_pinAction, &QAction::triggered, this, &HomeManagerPrivate::togglePinned);
    ...
    connect(m_gotoPreviousDocHistoryAction, &QAction::triggered,
            this, &HomeManagerPrivate::gotoPreviousDocHistory);
    ...
    connect(m_gotoNextDocHistoryAction, &QAction::triggered,
            this, &HomeManagerPrivate::gotoNextDocHistory);
    ...
    connect(m_goBackAction, &QAction::triggered,
            m_instance, &HomeManager::goBackInNavigationHistory);
    ...
    connect(m_goForwardAction, &QAction::triggered,
            m_instance, &HomeManager::goForwardInNavigationHistory);
    ...
    connect(m_gotoLastEditAction, &QAction::triggered,
            this, &HomeManagerPrivate::gotoLastEditLocation);
    ...
    mwindow->addAction(cmd, Constants::G_WINDOW_SPLIT);
    connect(m_splitSideBySideAction, &QAction::triggered, m_instance, &HomeManager::splitSideBySide);
    ...
    connect(m_removeCurrentSplitAction, &QAction::triggered,
            this, &HomeManagerPrivate::removeCurrentSplit);
    ...
    connect(m_removeAllSplitsAction, &QAction::triggered,
            this, &HomeManagerPrivate::removeAllSplits);
    ...
    connect(m_gotoPreviousSplitAction, &QAction::triggered,
            this, &HomeManagerPrivate::gotoPreviousSplit);
    ...
    mwindow->addAction(cmd, Constants::G_WINDOW_SPLIT);
    connect(m_gotoNextSplitAction, &QAction::triggered, this, &HomeManagerPrivate::gotoNextSplit);
    ...
    // other setup
    auto mainEditorArea = new HomeArea();
    ...
    mainEditorArea->hide();
    connect(mainEditorArea, &HomeArea::windowTitleNeedsUpdate,
            this, &HomeManagerPrivate::updateWindowTitle);
    connect(mainEditorArea, &QObject::destroyed, this, &HomeManagerPrivate::editorAreaDestroyed);
    ...
    m_autoSaveTimer->setObjectName("HomeManager::m_autoSaveTimer");
    connect(m_autoSaveTimer, &QTimer::timeout, this, &HomeManagerPrivate::autoSave);
    updateAutoSave();
    ...
    globalMacroExpander()->registerFileVariables(kCurrentDocumentPrefix, tr("Current document"),
        [] {
            IDocument *document = HomeManager::currentDocument();
...
void HomeManagerPrivate::extensionsInitialized()
{
    // Do not ask for files to save.
    // MainWindow::closeEvent has already done that.
    ICore::addPreCloseListener([]() -> bool { return HomeManager::closeAllEditors(false); });
}

HomeManagerPrivate *HomeManagerPrivate::instance()
{
    return d;
}

HomeArea *HomeManagerPrivate::mainEditorArea()
{
    return d->m_editorAreas.at(0);
}

bool HomeManagerPrivate::skipOpeningBigTextFile(const FilePath &filePath)
{
    ...
    if (fileSizeInMB > d->m_settings.bigFileSizeLimitInMB
        && fileSize < HomeManager::maxTextFileSize()) {
        const QString title = HomeManager::tr("Continue Opening Huge Text File?");
        const QString text = HomeManager::tr(
...
IEditor *HomeManagerPrivate::openEditor(HomeView *view, const FilePath &filePath, Id editorId,
                                          HomeManager::OpenEditorFlags flags, bool *newEditor)
{
    ...
    if (!editors.isEmpty()) {
        ...
        if (flags & HomeManager::SwitchSplitIfAlreadyVisible) {
            for (IEditor *ed : editors) {
                HomeView *v = viewForEditor(ed);
    ...
    HomeFactoryList factories = HomeManagerPrivate::findFactories(Id(), fn);
    if (factories.isEmpty()) {
        Utils::MimeType mimeType = Utils::mimeTypeForFile(fn);
        QMessageBox msgbox(QMessageBox::Critical, HomeManager::tr("File Error"),
...
IEditor *HomeManagerPrivate::openEditorAt(HomeView *view,
                                            const Link &link,
                                            Id editorId,
                                            HomeManager::OpenEditorFlags flags,
                                            bool *newEditor)
{
    HomeManager::cutForwardNavigationHistory();
    HomeManager::addCurrentPositionToNavigationHistory();
    HomeManager::OpenEditorFlags tempFlags = flags | HomeManager::IgnoreNavigationHistory;
...
IEditor *HomeManagerPrivate::openEditorWith(const FilePath &filePath, Id editorId)
{
    // close any open editors that have this file open
    // remember the views to open new editors in there
    QList<HomeView *> views;
    QList<IEditor *> editorsOpenForFile = DocumentModel::editorsForFilePath(filePath);
    foreach (IEditor *openEditor, editorsOpenForFile) {
        HomeView *view = HomeManagerPrivate::viewForEditor(openEditor);
        if (view && view->currentEditor() == openEditor) // visible
            views.append(view);
    }
    if (!HomeManager::closeEditors(editorsOpenForFile)) // don't open if cancel was pressed
        return nullptr;

    IEditor *openedEditor = nullptr;
    if (views.isEmpty()) {
        openedEditor = HomeManager::openEditor(filePath, editorId);
    } else {
        if (HomeView *currentView = HomeManagerPrivate::currentEditorView()) {
            if (views.removeOne(currentView))
                views.prepend(currentView); // open editor in current view first
        }
        HomeManager::OpenEditorFlags flags;
        foreach (HomeView *view, views) {
            IEditor *editor = HomeManagerPrivate::openEditor(view, filePath, editorId, flags);
            ...
            flags |= HomeManager::DoNotChangeCurrentEditor;
...
IEditor *HomeManagerPrivate::activateEditorForDocument(HomeView *view, IDocument *document,
                                                         HomeManager::OpenEditorFlags flags)
{
    Q_ASSERT(view);
...
HomeView *HomeManagerPrivate::viewForEditor(IEditor *editor)
{
    QWidget *w = editor->widget();
    while (w) {
        w = w->parentWidget();
        if (auto view = qobject_cast<HomeView *>(w))
            return view;
    }
    return nullptr;
}

MakeWritableResult HomeManagerPrivate::makeFileWritable(IDocument *document)
{
    if (!document)
        return Failed;
...
/*!
    ...
    \internal
*/
void HomeManagerPrivate::doEscapeKeyFocusMoveMagic()
{
    // use cases to cover:
    ...
    HomeView *editorView = currentEditorView();
...
//    }
    ...
    if (!editorViewActive && !editorViewVisible) {
        ...
        ModeManager::activateMode(Id(Constants::MODE_HOME));
...
OpenEditorsWindow *HomeManagerPrivate::windowPopup()
{
    return d->m_windowPopup;
}

void HomeManagerPrivate::showPopupOrSelectDocument()
{
    if (QApplication::keyboardModifiers() == Qt::NoModifier) {
        ...
        HomeArea *activeEditorArea = nullptr;
        foreach (HomeArea *area, d->m_editorAreas) {
        ...
        if (!activeEditorArea)
            activeEditorArea = findEditorArea(HomeManagerPrivate::currentEditorView());
...
Id HomeManagerPrivate::getOpenWithEditorId(const QString &fileName, bool *isExternalEditor)
{
    ...
    // Built-in
    const HomeFactoryList editors = IEditorFactory::preferredEditorFactories(fileName);
    ...
    const ExternalEditorList exEditors = IExternalHome::externalEditors(mt);
...
{
    const HomeFactoryList factories = IEditorFactory::allEditorFactories();
...
void HomeManagerPrivate::saveSettings()
{
    ...
    qsettings->setValueWithDefault(preferredHomeFactoriesKey,
                                   toMap(userPreferredEditorFactories()));
}

void HomeManagerPrivate::readSettings()
{
    Settings def;
    ...
    const QHash<Utils::MimeType, IEditorFactory *> preferredEditorFactories = fromMap(
        qs->value(preferredHomeFactoriesKey).toMap());
...
void HomeManagerPrivate::setAutoSaveEnabled(bool enabled)
{
    d->m_settings.autoSaveEnabled = enabled;
    updateAutoSave();
}

bool HomeManagerPrivate::autoSaveEnabled()
{
    return d->m_settings.autoSaveEnabled;
}

void HomeManagerPrivate::setAutoSaveInterval(int interval)
{
    d->m_settings.autoSaveInterval = interval;
    updateAutoSave();
}

int HomeManagerPrivate::autoSaveInterval()
{
    return d->m_settings.autoSaveInterval;
}

void HomeManagerPrivate::setAutoSuspendEnabled(bool enabled)
{
    d->m_settings.autoSuspendEnabled = enabled;
}

bool HomeManagerPrivate::autoSuspendEnabled()
{
    return d->m_settings.autoSuspendEnabled;
}

void HomeManagerPrivate::setAutoSuspendMinDocumentCount(int count)
{
    d->m_settings.autoSuspendMinDocumentCount = count;
}

int HomeManagerPrivate::autoSuspendMinDocumentCount()
{
    return d->m_settings.autoSuspendMinDocumentCount;
}

bool HomeManagerPrivate::warnBeforeOpeningBigFilesEnabled()
{
    return d->m_settings.warnBeforeOpeningBigFilesEnabled;
}

void HomeManagerPrivate::setWarnBeforeOpeningBigFilesEnabled(bool enabled)
{
    d->m_settings.warnBeforeOpeningBigFilesEnabled = enabled;
}

int HomeManagerPrivate::bigFileSizeLimit()
{
    return d->m_settings.bigFileSizeLimitInMB;
}

void HomeManagerPrivate::setMaxRecentFiles(int count)
{
    d->m_settings.maxRecentFiles = count;
}

int HomeManagerPrivate::maxRecentFiles()
{
    return d->m_settings.maxRecentFiles;
}

void HomeManagerPrivate::setBigFileSizeLimit(int limitInMB)
{
    d->m_settings.bigFileSizeLimitInMB = limitInMB;
}

HomeFactoryList HomeManagerPrivate::findFactories(Id editorId, const QString &fileName)
{
    if (debugEditorManager)
        qDebug() << Q_FUNC_INFO << editorId.name() << fileName;

    HomeFactoryList factories;
    if (!editorId.isValid()) {
...
IEditor *HomeManagerPrivate::createEditor(IEditorFactory *factory, const QString &fileName)
{
    if (!factory)
        return nullptr;
    ...
    if (editor) {
        ...
        connect(editor->document(), &IDocument::changed, d, &HomeManagerPrivate::handleDocumentStateChange);
...
void HomeManagerPrivate::addEditor(IEditor *editor)
{
    if (!editor)
        return;
    ...
    QMetaObject::invokeMethod(d, &HomeManagerPrivate::autoSuspendDocuments, Qt::QueuedConnection);
}

void HomeManagerPrivate::removeEditor(IEditor *editor, bool removeSuspendedEntry)
{
...
IEditor *HomeManagerPrivate::placeEditor(HomeView *view, IEditor *editor)
{
    ...
    if (HomeView *sourceView = viewForEditor(editor)) {
        ...
        if (editor != sourceView->currentEditor() || !duplicateSupported) {
            ...
            if (!sourceView->currentEditor()) {
                HomeView *replacementView = nullptr;
...
IEditor *HomeManagerPrivate::duplicateEditor(IEditor *editor)
{
...
IEditor *HomeManagerPrivate::activateEditor(HomeView *view, IEditor *editor,
                                              HomeManager::OpenEditorFlags flags)
{
    Q_ASSERT(view);
    ...
    if (!(flags & HomeManager::DoNotChangeCurrentEditor)) {
        setCurrentEditor(editor, (flags & HomeManager::IgnoreNavigationHistory));
        if (!(flags & HomeManager::DoNotMakeVisible)) {
            // switch to design mode?
            if (!(flags & HomeManager::DoNotSwitchToDesignMode) && editor->isDesignModePreferred()) {
                ModeManager::activateMode(Constants::MODE_DESIGN);
                ModeManager::setFocusToCurrentMode();
            } else {
                if (!(flags & HomeManager::DoNotSwitchToEditMode)) {
                    int index;
                    findEditorArea(view, &index);
                    if (index == 0) // main window --> we might need to switch mode
                        if (!editor->widget()->isVisible())
                            ModeManager::activateMode(Constants::MODE_HOME);
                }
                editor->widget()->setFocus();
                if (!(flags & HomeManager::DoNotRaise))
                    ICore::raiseWindow(editor->widget());
            }
        }
    } else if (!(flags & HomeManager::DoNotMakeVisible)) {
        view->setCurrentEditor(editor);
    }
    return editor;
}

bool HomeManagerPrivate::activateEditorForEntry(HomeView *view, DocumentModel::Entry *entry,
                                                  HomeManager::OpenEditorFlags flags)
{
...
void HomeManagerPrivate::closeEditorOrDocument(IEditor *editor)
{
    QTC_ASSERT(editor, return);
    QList<IEditor *> visible = HomeManager::visibleEditors();
    if (Utils::contains(visible,
                        [&editor](IEditor *other) {
                            return editor != other && other->document() == editor->document();
                        })) {
        HomeManager::closeEditors({editor});
    } else {
        HomeManager::closeDocuments({editor->document()});
    }
}

bool HomeManagerPrivate::closeEditors(const QList<IEditor*> &editors, CloseFlag flag)
{
    if (editors.isEmpty())
        return true;
    ...
    HomeView *currentView = currentEditorView();
    ...
    // a suspended editor
    QList<HomeView *> views = editorsPerView.keys();
    Utils::sort(views, [editorsPerView, currentView](HomeView *a, HomeView *b) {
        if (a == b)
            return false;
    ...
    for (HomeView *view : qAsConst(views)) {
        ...
        for (IEditor *editor : qAsConst(editors)) {
            if (editor == viewCurrentEditor && view == views.last()) {
                ...
                HomeManager::OpenEditorFlags flags = view != currentView
                        ? HomeManager::DoNotChangeCurrentEditor : HomeManager::NoFlags;
                        ...
                        if (!documents.isEmpty()) {
                            if (IDocument *document = documents.last()->document) {
                                ...
                                if (view == currentView && document == editor->document())
                                    flags = HomeManager::DoNotSwitchToDesignMode;
...
void HomeManagerPrivate::activateView(HomeView *view)
{
...
void HomeManagerPrivate::restoreEditorState(IEditor *editor)
{
    QTC_ASSERT(editor, return);
...
int HomeManagerPrivate::visibleDocumentsCount()
{
    const QList<IEditor *> editors = HomeManager::visibleEditors();
...
void HomeManagerPrivate::setCurrentEditor(IEditor *editor, bool ignoreNavigationHistory)
{
    if (editor)
    ...
    if (editor) {
        ...
        HomeView::updateEditorHistory(editor, d->m_globalHistory);
    }
    updateActions();
    emit m_instance->currentEditorChanged(editor);
}

void HomeManagerPrivate::setCurrentView(HomeView *view)
{
    if (view == d->m_currentView)
        return;

    HomeView *old = d->m_currentView;
...
HomeArea *HomeManagerPrivate::findEditorArea(const HomeView *view, int *areaIndex)
{
    SplitterOrView *current = view->parentSplitterOrView();
    while (current) {
        if (auto area = qobject_cast<HomeArea *>(current)) {
...
void HomeManagerPrivate::closeView(HomeView *view)
{
    if (!view)
        return;
    ...
    HomeView *newCurrent = splitter->findFirstView();
    if (newCurrent)
...
/*!
    ...
    \internal
*/
const QList<IEditor *> HomeManagerPrivate::emptyView(HomeView *view)
{
    if (!view)
        return {};
    ...
    for (IEditor *editor : editors) {
        if (DocumentModel::editorsForDocument(editor->document()).size() == 1) {
            ...
            if (HomeManager::currentEditor() == editor) {
...
/*!
    ...
    \internal
*/
void HomeManagerPrivate::deleteEditors(const QList<IEditor *> &editors)
{
    if (!editors.isEmpty()) {
...
HomeWindow *HomeManagerPrivate::createEditorWindow()
{
    auto win = new HomeWindow;
    HomeArea *area = win->editorArea();
    d->m_editorAreas.append(area);
    connect(area, &QObject::destroyed, d, &HomeManagerPrivate::editorAreaDestroyed);
    return win;
}

void HomeManagerPrivate::splitNewWindow(HomeView *view)
{
    IEditor *editor = view->currentEditor();
    IEditor *newEditor = nullptr;
    const QByteArray state = editor ? editor->saveState() : QByteArray();
    if (editor && editor->duplicateSupported())
        newEditor = HomeManagerPrivate::duplicateEditor(editor);
    else
        newEditor = editor; // move to the new view

    HomeWindow *win = createEditorWindow();
    win->show();
    ICore::raiseWindow(win);
    if (newEditor) {
        activateEditor(win->editorArea()->view(), newEditor, HomeManager::IgnoreNavigationHistory);
        // possibly adapts old state to new layout
...
IEditor *HomeManagerPrivate::pickUnusedEditor(HomeView **foundView)
{
    foreach (IEditor *editor, DocumentModel::editorsForOpenedDocuments()) {
        HomeView *view = viewForEditor(editor);
...
void HomeManagerPrivate::addDocumentToRecentFiles(IDocument *document)
{
...
void HomeManagerPrivate::updateAutoSave()
{
...
void HomeManagerPrivate::updateMakeWritableWarning()
{
    IDocument *document = HomeManager::currentDocument();
...
void HomeManagerPrivate::setupSaveActions(IDocument *document, QAction *saveAction,
                                            QAction *saveAsAction, QAction *revertToSavedAction)
{
    ...
    if (document && !document->displayName().isEmpty()) {
        ...
        saveAction->setText(HomeManager::tr("&Save"));
        saveAsAction->setText(HomeManager::tr("Save &As..."));
        revertToSavedAction->setText(HomeManager::tr("Revert to Saved"));
    }
}

void HomeManagerPrivate::updateActions()
{
    IDocument *curDocument = HomeManager::currentDocument();
    ...
    HomeView *view  = currentEditorView();
...
void HomeManagerPrivate::updateWindowTitleForDocument(IDocument *document, QWidget *window)
{
...
void HomeManagerPrivate::updateWindowTitle()
{
    HomeArea *mainArea = mainEditorArea();
    IDocument *document = mainArea->currentDocument();
    updateWindowTitleForDocument(document, mainArea->window());
}

void HomeManagerPrivate::gotoNextDocHistory()
{
...
void HomeManagerPrivate::gotoPreviousDocHistory()
{
    OpenEditorsWindow *dialog = windowPopup();
    if (dialog->isVisible()) {
...
void HomeManagerPrivate::gotoLastEditLocation()
{
    currentEditorView()->goToEditLocation(d->m_globalLastEditLocation);
}

void HomeManagerPrivate::gotoNextSplit()
{
    HomeView *view = currentEditorView();
    if (!view)
        return;
    HomeView *nextView = view->findNextView();
    if (!nextView) {
        // we are in the "last" view in this editor area
        int index = -1;
        HomeArea *area = findEditorArea(view, &index);
...
void HomeManagerPrivate::gotoPreviousSplit()
{
    HomeView *view = currentEditorView();
    if (!view)
        return;
    HomeView *prevView = view->findPreviousView();
    if (!prevView) {
...
void HomeManagerPrivate::makeCurrentEditorWritable()
{
    if (IDocument* doc = HomeManager::currentDocument())
        makeFileWritable(doc);
}

void HomeManagerPrivate::setPlaceholderText(const QString &text)
{
...
QString HomeManagerPrivate::placeholderText()
{
    return d->m_placeholderText;
}

void HomeManagerPrivate::vcsOpenCurrentEditor()
{
    IDocument *document = HomeManager::currentDocument();
    if (!document)
        return;
...
void HomeManagerPrivate::editorAreaDestroyed(QObject *area)
{
    ...
    HomeView *focusView = focusSplitterOrView->findFirstView(); // can be just focusSplitterOrView
    QTC_ASSERT(focusView, focusView = newActiveArea->findFirstView());
    QTC_ASSERT(focusView, return);
    HomeManagerPrivate::activateView(focusView);
}

void HomeManagerPrivate::autoSave()
{
    QStringList errors;
...
void HomeManagerPrivate::handleContextChange(const QList<IContext *> &context)
{
    if (debugEditorManager)
        qDebug() << Q_FUNC_INFO;
    ...
    if (editor && editor != d->m_currentEditor) {
        ...
        QMetaObject::invokeMethod(d, &HomeManagerPrivate::setCurrentEditorFromContextChange,
                                  Qt::QueuedConnection);
    } else {
        updateActions();
    }
...
void HomeManagerPrivate::copyFilePathFromContextMenu()
{
...
void HomeManagerPrivate::copyLocationFromContextMenu()
{
...
void HomeManagerPrivate::copyFileNameFromContextMenu()
{
    if (!d->m_contextMenuEntry)
        return;
...
void HomeManagerPrivate::saveDocumentFromContextMenu()
{
...
void HomeManagerPrivate::saveDocumentAsFromContextMenu()
{
    IDocument *document = d->m_contextMenuEntry ? d->m_contextMenuEntry->document : nullptr;
    if (document)
...
void HomeManagerPrivate::revertToSavedFromContextMenu()
{
    IDocument *document = d->m_contextMenuEntry ? d->m_contextMenuEntry->document : nullptr;
    if (document)
...
void HomeManagerPrivate::closeEditorFromContextMenu()
{
    if (d->m_contextMenuEditor) {
        ...
        IDocument *document = d->m_contextMenuEntry ? d->m_contextMenuEntry->document : nullptr;
        if (document)
            HomeManager::closeDocuments({document});
    }
}

void HomeManagerPrivate::closeOtherDocumentsFromContextMenu()
{
    IDocument *document = d->m_contextMenuEntry ? d->m_contextMenuEntry->document : nullptr;
    HomeManager::closeOtherDocuments(document);
}

bool HomeManagerPrivate::saveDocument(IDocument *document)
{
    if (!document)
        return false;
...
bool HomeManagerPrivate::saveDocumentAs(IDocument *document)
{
    if (!document)
        return false;
    ...
    if (absoluteFilePath != document->filePath()) {
        ...
        if (otherDocument)
            HomeManager::closeDocuments({otherDocument}, false);
    }
...
void HomeManagerPrivate::closeAllEditorsExceptVisible()
{
    ...
    foreach (IEditor *editor, HomeManager::visibleEditors())
        documentsToClose.removeAll(editor->document());
    HomeManager::closeDocuments(documentsToClose, true);
}

void HomeManagerPrivate::revertToSaved(IDocument *document)
{
    if (!document)
        return;
    const QString fileName =  document->filePath().toString();
...
void HomeManagerPrivate::autoSuspendDocuments()
{
    if (!d->m_settings.autoSuspendEnabled)
        return;

    auto visibleDocuments = Utils::transform<QSet>(HomeManager::visibleEditors(),
                                                   &IEditor::document);
...
void HomeManagerPrivate::openTerminal()
{
    if (!d->m_contextMenuEntry || d->m_contextMenuEntry->fileName().isEmpty())
        return;
...
void HomeManagerPrivate::findInDirectory()
{
    if (!d->m_contextMenuEntry || d->m_contextMenuEntry->fileName().isEmpty())
        return;
...
void HomeManagerPrivate::togglePinned()
{
    if (!d->m_contextMenuEntry || d->m_contextMenuEntry->fileName().isEmpty())
        return;
...
void HomeManagerPrivate::split(Qt::Orientation orientation)
{
    HomeView *view = currentEditorView();
...
void HomeManagerPrivate::removeCurrentSplit()
{
    HomeView *viewToClose = currentEditorView();

    QTC_ASSERT(viewToClose, return);
    QTC_ASSERT(!qobject_cast<HomeArea *>(viewToClose->parentSplitterOrView()), return);

    closeView(viewToClose);
    updateActions();
}

void HomeManagerPrivate::removeAllSplits()
{
    HomeView *view = currentEditorView();
    QTC_ASSERT(view, return);
    HomeArea *currentArea = findEditorArea(view);
    QTC_ASSERT(currentArea, return);
    currentArea->unsplitAll();
}

void HomeManagerPrivate::setCurrentEditorFromContextChange()
{
...
HomeView *HomeManagerPrivate::currentEditorView()
{
    HomeView *view = d->m_currentView;
    if (!view) {
        if (d->m_currentEditor) {
            view = HomeManagerPrivate::viewForEditor(d->m_currentEditor);
...
/*!
    \internal
*/
HomeManager::HomeManager(QObject *parent) :
    QObject(parent)
{
    m_instance = this;
    d = new HomeManagerPrivate(this);
    d->init();
}
...
/*!
    ...
    \sa currentEditor()
*/
IDocument *HomeManager::currentDocument()
{
...
/*!
    ...
    \sa currentDocument()
*/
IEditor *HomeManager::currentEditor()
{
...
*/
bool HomeManager::closeAllEditors(bool askAboutModifiedEditors)
{
...
*/
void HomeManager::closeOtherDocuments(IDocument *document)
{
    DocumentModelPrivate::removeAllSuspendedEntries(DocumentModelPrivate::DoNotRemovePinnedFiles);
    QList<IDocument *> documentsToClose = DocumentModel::openedDocuments();
    // Remove all pinned files from the list of files to close.
...
/*!
    ...
    Returns whether all editors were closed.
*/
bool HomeManager::closeAllDocuments()
{
    ...
    return HomeManager::closeDocuments(entriesToClose);
}

/*!
    \internal
*/
void HomeManager::slotCloseCurrentEditorOrDocument()
{
    if (!d->m_currentEditor)
        return;
...
*/
void HomeManager::closeOtherDocuments()
{
...
*/
void HomeManager::addSaveAndCloseEditorActions(QMenu *contextMenu, DocumentModel::Entry *entry,
                                                 IEditor *editor)
{
    ...
    HomeManagerPrivate::setupSaveActions(document,
    ...
    d->m_closeAllEditorsExceptVisibleContextAction->setEnabled(
                HomeManagerPrivate::visibleDocumentsCount() < DocumentModel::entries().count());
...
/*!
    ...
    \a contextMenu.
*/
void HomeManager::addPinEditorActions(QMenu *contextMenu, DocumentModel::Entry *entry)
{
    const QString quotedDisplayName = entry ? Utils::quoteAmpersands(entry->displayName()) : QString();
    if (entry) {
...
*/
void HomeManager::addNativeDirAndOpenWithActions(QMenu *contextMenu, DocumentModel::Entry *entry)
{
    QTC_ASSERT(contextMenu, return);
    d->m_contextMenuEntry = entry;
...
*/
void HomeManager::populateOpenWithMenu(QMenu *menu, const QString &fileName)
{
    ...
    const Utils::MimeType mt = Utils::mimeTypeForFile(fileName);
    const ExternalEditorList extEditors = IExternalHome::externalEditors(mt);
    ...
    if (anyMatches) {
        ...
        foreach (IEditorFactory *editorFactory, factories) {
            ...
            connect(action, &QAction::triggered, d,
                    [fileName, editorId]() {
                        HomeManagerPrivate::openEditorWith(FilePath::fromString(fileName), editorId);
                    }, Qt::QueuedConnection);
        }
        // Add all suitable external editors
        foreach (IExternalHome *externalEditor, extEditors) {
            ...
            connect(action, &QAction::triggered, [fileName, editorId]() {
                HomeManager::openExternalEditor(FilePath::fromString(fileName), editorId);
            });
        }
    }
...
*/
IDocument::ReloadSetting HomeManager::reloadSetting()
{
...
*/
void HomeManager::setReloadSetting(IDocument::ReloadSetting behavior)
{
...
*/
void HomeManager::saveDocument()
{
    HomeManagerPrivate::saveDocument(currentDocument());
}

/*!
    Saves the current document under a different file name.
*/
void HomeManager::saveDocumentAs()
{
    HomeManagerPrivate::saveDocumentAs(currentDocument());
}

/*!
    Reverts the current document to its last saved state.
*/
void HomeManager::revertToSaved()
{
    HomeManagerPrivate::revertToSaved(currentDocument());
}
...
*/
bool HomeManager::closeDocuments(const QList<DocumentModel::Entry *> &entries)
{
    QList<IDocument *> documentsToClose;
...
/*!
    ...
    \sa closeDocuments()
*/
bool HomeManager::closeEditors(const QList<IEditor*> &editorsToClose, bool askAboutModifiedEditors)
{
    return HomeManagerPrivate::closeEditors(editorsToClose,
                                              askAboutModifiedEditors ? HomeManagerPrivate::CloseFlag::CloseWithAsking
                                                                      : HomeManagerPrivate::CloseFlag::CloseWithoutAsking);
}
...
*/
void HomeManager::activateEditorForEntry(DocumentModel::Entry *entry, OpenEditorFlags flags)
{
    HomeManagerPrivate::activateEditorForEntry(HomeManagerPrivate::currentEditorView(),
                                                 entry, flags);
}
...
/*!
    ...
    \sa currentEditor()
*/
void HomeManager::activateEditor(IEditor *editor, OpenEditorFlags flags)
{
    QTC_ASSERT(editor, return);
    HomeView *view = HomeManagerPrivate::viewForEditor(editor);
    // an IEditor doesn't have to belong to a view, it might be kept in storage by the editor model
    if (!view)
        view = HomeManagerPrivate::currentEditorView();
    HomeManagerPrivate::activateEditor(view, editor, flags);
}
...
/*!
    ...
    specified \a flags.
*/
IEditor *HomeManager::activateEditorForDocument(IDocument *document, OpenEditorFlags flags)
{
    return HomeManagerPrivate::activateEditorForDocument(HomeManagerPrivate::currentEditorView(), document, flags);
}
...
*/
IEditor *HomeManager::openEditor(const FilePath &filePath, Id editorId,
                                   OpenEditorFlags flags, bool *newEditor)
{
    if (flags & HomeManager::OpenInOtherSplit)
        HomeManager::gotoOtherSplit();

    return HomeManagerPrivate::openEditor(HomeManagerPrivate::currentEditorView(),
                                            filePath, editorId, flags, newEditor);
}

IEditor *HomeManager::openEditor(const QString &fileName, Id editorId,
                                   OpenEditorFlags flags, bool *newEditor)
{
...
/*!
    ...
    \sa IEditor::gotoLine()
*/
IEditor *HomeManager::openEditorAt(const Link &link,
                                     Id editorId,
                                     ...
                                     bool *newEditor)
{
    ...
    return HomeManagerPrivate::openEditorAt(HomeManagerPrivate::currentEditorView(),
                                              link,
                                              editorId,
                                              flags,
                                              newEditor);
...
IEditor *HomeManager::openEditorAt(const QString &fileName, int line, int column,
...
*/
bool HomeManager::isAutoSaveFile(const QString &filePath)
{
...
/*!
    ...
    \sa openEditor()
*/
bool HomeManager::openExternalEditor(const FilePath &filePath, Id editorId)
{
    IExternalHome *ee = Utils::findOrDefault(IExternalHome::allExternalEditors(),
                                               Utils::equal(&IExternalHome::id, editorId));
    if (!ee)
        return false;
...
/*!
    ...
    accepted.
*/
void HomeManager::addCloseEditorListener(const std::function<bool (IEditor *)> &listener)
{
...
*/
QStringList HomeManager::getOpenFileNames()
{
...
QString HomeManager::getOpenFolderName()
{
    return DocumentManager::getOpenFolderName(QString());
}
...
/*!
    ...
    \sa clearUniqueId()
*/
IEditor *HomeManager::openEditorWithContents(Id editorId,
                                        ...
                                        OpenEditorFlags flags)
{
    ...
    HomeFactoryList factories = HomeManagerPrivate::findFactories(editorId, title);
    if (factories.isEmpty())
        return nullptr;

    edt = HomeManagerPrivate::createEditor(factories.first(), title);
    if (!edt)
        return nullptr;
    ...
    HomeManagerPrivate::addEditor(edt);
    activateEditor(edt, flags);
    return edt;
}
...
*/
bool HomeManager::skipOpeningBigTextFile(const FilePath &filePath)
{
    return HomeManagerPrivate::skipOpeningBigTextFile(filePath);
}
...
/*!
    ...
    \sa openEditorWithContents()
*/
void HomeManager::clearUniqueId(IDocument *document)
{
...
*/
bool HomeManager::saveDocument(IDocument *document)
{
    return HomeManagerPrivate::saveDocument(document);
}

/*!
    \internal
*/
bool HomeManager::hasSplitter()
{
    HomeView *view = HomeManagerPrivate::currentEditorView();
    QTC_ASSERT(view, return false);
    HomeArea *area = HomeManagerPrivate::findEditorArea(view);
...
*/
QList<IEditor*> HomeManager::visibleEditors()
{
    QList<IEditor *> editors;
    foreach (HomeArea *area, d->m_editorAreas) {
        if (area->isSplitter()) {
            HomeView *firstView = area->findFirstView();
            HomeView *view = firstView;
            if (view) {
                do {
...
*/
bool HomeManager::closeDocuments(const QList<IDocument *> &documents, bool askAboutModifiedEditors)
{
...
*/
void HomeManager::addCurrentPositionToNavigationHistory(const QByteArray &saveState)
{
    HomeManagerPrivate::currentEditorView()->addCurrentPositionToNavigationHistory(saveState);
...
*/
void HomeManager::setLastEditLocation(const IEditor* editor)
{
    IDocument *document = editor->document();
    if (!document)
        return;
...
*/
void HomeManager::cutForwardNavigationHistory()
{
    HomeManagerPrivate::currentEditorView()->cutForwardNavigationHistory();
    HomeManagerPrivate::updateActions();
}
...
/*!
    ...
    \sa addCurrentPositionToNavigationHistory()
*/
void HomeManager::goBackInNavigationHistory()
{
    HomeManagerPrivate::currentEditorView()->goBackInNavigationHistory();
    HomeManagerPrivate::updateActions();
    return;
}
...
/*!
    ...
    \sa addCurrentPositionToNavigationHistory()
*/
void HomeManager::goForwardInNavigationHistory()
{
    HomeManagerPrivate::currentEditorView()->goForwardInNavigationHistory();
    HomeManagerPrivate::updateActions();
}

HomeWindow *windowForEditorArea(HomeArea *area)
{
    return qobject_cast<HomeWindow *>(area->window());
}

QVector<HomeWindow *> editorWindows(const QList<HomeArea *> &areas)
{
    QVector<HomeWindow *> result;
    for (HomeArea *area : areas)
        if (HomeWindow *window = windowForEditorArea(area))
...
/*!
    ...
    \sa restoreState()
*/
QByteArray HomeManager::saveState()
{
    QByteArray bytes;
    QDataStream stream(&bytes, QIODevice::WriteOnly);

    stream << QByteArray("HomeManagerV5");
    ...
    // windows
    const QVector<HomeWindow *> windows = editorWindows(d->m_editorAreas);
    const QVector<QVariantHash> windowStates = Utils::transform(windows, &HomeWindow::saveState);
    stream << windowStates;
    return bytes;
}
...
/*!
    ...
    \sa saveState()
*/
bool HomeManager::restoreState(const QByteArray &state)
{
    ...
    if (d->m_editorAreas.first()->isSplitter())
        HomeManagerPrivate::removeAllSplits();
    QDataStream stream(state);

    QByteArray version;
    stream >> version;

    const bool isVersion5 = version == "HomeManagerV5";
    if (version != "HomeManagerV4" && !isVersion5)
        return false;
    ...
    if (!stream.atEnd()) { // safety for settings from Qt Creator 4.5 and earlier
        ...
        for (const QVariantHash &windowState : qAsConst(windowStates)) {
            HomeWindow *window = d->createEditorWindow();
    ...
    } else if (Internal::HomeView *view = HomeManagerPrivate::currentEditorView()) {
...
/*!
    \internal
*/
void HomeManager::showEditorStatusBar(const QString &id,
                                        ...
                                        const std::function<void()> &function)
{
    ...
    HomeManagerPrivate::currentEditorView()->showEditorStatusBar(
...
/*!
    \internal
*/
void HomeManager::hideEditorStatusBar(const QString &id)
{
    // TODO: what if the current editor view betwenn show and hideEditorStatusBar changed?
    HomeManagerPrivate::currentEditorView()->hideEditorStatusBar(id);
}

/*!
    Returns the default text codec as the user specified in the settings.
*/
QTextCodec *HomeManager::defaultTextCodec()
{
...
*/
TextFileFormat::LineTerminationMode HomeManager::defaultLineEnding()
{
    QSettings *settings = ICore::settings();
...
*/
void HomeManager::splitSideBySide()
{
    HomeManagerPrivate::split(Qt::Horizontal);
}
 ...
 */
void HomeManager::gotoOtherSplit()
{
    HomeView *view = HomeManagerPrivate::currentEditorView();
    if (!view)
        return;
    ...
    if (!nextView) {
        // we are in the "last" view in this editor area
        int index = -1;
        HomeArea *area = HomeManagerPrivate::findEditorArea(view, &index);
        QTC_ASSERT(area, return);
        QTC_ASSERT(index >= 0 && index < d->m_editorAreas.size(), return);
    ...
    if (nextView)
        HomeManagerPrivate::activateView(nextView);
}

/*!
    Returns the maximum file size that should be opened in a text editor.
*/
qint64 HomeManager::maxTextFileSize()
{
    return qint64(3) << 24;
}
...
*/
void HomeManager::setWindowTitleAdditionHandler(WindowTitleHandler handler)
{
...
*/
void HomeManager::setSessionTitleHandler(WindowTitleHandler handler)
{
...
/*!
    \internal
*/
void HomeManager::updateWindowTitles()
{
    foreach (HomeArea *area, d->m_editorAreas)
...
/*!
    \internal
*/
void HomeManager::setWindowTitleVcsTopicHandler(WindowTitleHandler handler)
{
