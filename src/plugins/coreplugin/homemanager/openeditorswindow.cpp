...
Q_DECLARE_METATYPE(Core::Internal::HomeView*)
...
void OpenEditorsWindow::setEditors(const QList<EditLocation> &globalHistory, HomeView *view)
{
    m_editorList->clear();
...
void OpenEditorsWindow::selectEditor(QTreeWidgetItem *item)
{
    ...
    auto view = item->data(0, int(Role::View)).value<HomeView *>();
    if (!HomeManagerPrivate::activateEditorForEntry(view, entry))
        delete item;
}
...
void OpenEditorsWindow::addHistoryItems(const QList<EditLocation> &history, HomeView *view,
                                        QSet<const DocumentModel::Entry *> &entriesDone)
{
...
void OpenEditorsWindow::addRemainingItems(HomeView *view,
                                          QSet<const DocumentModel::Entry *> &entriesDone)
{
