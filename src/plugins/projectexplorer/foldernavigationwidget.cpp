...
const char COMPUTERDIRECTORYROOT_ID[] = "A.Computer";
const char HOMEDIRECTORYROOT_ID[]     = "A.Home";
    ...
    m_crumbLabel(new DelayedFileCrumbLabel(this))
{
    ...
    // connections
    connect(Core::HomeManager::instance(), &Core::HomeManager::currentEditorChanged,
    ...
    const FolderNavigationWidgetFactory::RootDirectory &directory)
{
    ...
    if (m_autoSync) // we might find a better root for current selection now
        handleCurrentEditorChanged(Core::HomeManager::currentEditor());
}
...
void FolderNavigationWidget::setCurrentIndex(const int &index)
{
    m_rootSelector->setCurrentIndex(index);
}
...
void FolderNavigationWidget::openItem(const QModelIndex &index)
{
    ...
    Core::HomeManager::openEditor(path);
}
...
void FolderNavigationWidget::contextMenuEvent(QContextMenuEvent *ev)
{
    ...
    Core::HomeManager::addNativeDirAndOpenWithActions(&menu, &fakeEntry);
...
{
    m_instance = this;
    setDisplayName(tr("Working"));
    setPriority(400);
    setId(ProjectExplorer::Constants::FOLDER_NAVIGATION_WIDGET_FACTORY_ID);
    setActivationSequence(QKeySequence(Core::useMacShortcuts ? tr("Meta+Y,Meta+F")
                                                             : tr("Alt+Y,Alt+F")));
    insertRootDirectory({QLatin1String(COMPUTERDIRECTORYROOT_ID),
                         0 /*sortValue*/,
                         FolderNavigationWidget::tr("Computer"),
                         Utils::FilePath(),
                         Icons::DESKTOP_DEVICE_SMALL.icon()});
    insertRootDirectory({QLatin1String(HOMEDIRECTORYROOT_ID),
                         1 /*sortValue*/,
...
int FolderNavigationWidgetFactory::getRootDirectoryLength()
{
    return m_rootDirectories.length();
}
...
void FolderNavigationWidgetFactory::removeProject(const QString &id)
{
    removeRootDirectory(id);
}

void FolderNavigationWidgetFactory::setCurrentProject(const QString &id)
{
    auto fnw = qobject_cast<FolderNavigationWidget *>(Core::ICore::currentContextWidget());
    fnw->setCurrentIndex(rootIndex(id));
}

QList<QString> FolderNavigationWidgetFactory::getOpenedProjects()
{
    QList<QString> m_openedProjects;
    for (const RootDirectory &root : qAsConst(m_rootDirectories)) {
        if (root.id == QLatin1String(COMPUTERDIRECTORYROOT_ID) ||
            root.id == QLatin1String(HOMEDIRECTORYROOT_ID) ||
            root.id == QLatin1String(PROJECTSDIRECTORYROOT_ID)) {
            continue;
        }
        m_openedProjects.append(root.id);
    }
